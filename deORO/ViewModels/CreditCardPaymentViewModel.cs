...
namespace deORO.ViewModels
{
    public class CreditCardPaymentViewModel : BaseViewModel
    {
            ...
            set
            {
                try
                {
                    if (Global.CreditRound != 0.0m)
                    {
                        ...
                        creditRoudFee = (value * (Global.CreditRound / 100));
                    }
                    else
                    {
                        amountDue = value;
        ...
        private async void CardReaderData(CreditCardData cardData)
        {
            ...
            string transactionDetails = string.Format("A0|200|{0}|1|{1}", Global.AmountDue * 100, "Purchase using Credit Card");
            await Task.Run(() =>
            {
                ...
                {
                    cardProcessor.ProcessSale(cardData, Global.AmountDue, transactionDetails, zipCode);
                }
