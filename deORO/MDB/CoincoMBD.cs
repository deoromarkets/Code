...
namespace deORO.MDB
{
    ...
    public class CoincoMDB : deORO.Communication.ICommunicationType
    {
        ...
        [DllImport("cai_mdb_w32.dll")]
        public static extern long BVAL_IsOnline(IntPtr hDev);
        ...
        private void EnableBill(decimal value)
        {
            try
            {
                long b = BVAL_IsOnline(billHandle);
                int mx = 50, i = 0;
                while (b == 0 && i < mx ) {
                    BVAL_IsOnline(billHandle);
                    i += 1;
                }
                //System.Threading.Thread.Sleep(2000);
                BVAL_NoteEnable(billHandle, listNoteInfo.Find(x => x.Value == value).Index, 1);
                //Console.WriteLine(a);
            }
            catch { }
        }
        ...
        {
            long b = CHGR_IsOnline(coinHandle);
            int mx = 50, j= 0;
            while (b == 0 && j < mx)
            {
                CHGR_IsOnline(coinHandle);
                j += 1;
            }
            for (int i = 0; i <= maxTubes; i++)
            {
